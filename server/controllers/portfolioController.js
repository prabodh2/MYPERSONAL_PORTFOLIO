...
// Fallback seed data strictly following the resume specifications
const fallbackData = {
  projects: [
    {
      _id: "p1",
  ...
  try {
    if (mongoose.connection.readyState === 1) {
      ...
      if (dbProjects.length > 0) {
        return res.json({ success: true, data: dbProjects });
      }
    }
    return res.json({ success: true, data: fallbackData.projects });
  } catch (error) {
    return res.json({ success: true, data: fallbackData.projects });
  }
};
...
export const getExperiences = async (req, res) => {
  try {
    if (mongoose.connection.readyState === 1) {
      const dbExp = await Experience.find();
      if (dbExp.length > 0) return res.json({ success: true, data: dbExp });
    }
    return res.json({ success: true, data: fallbackData.experiences });
  } catch (error) {
    return res.json({ success: true, data: fallbackData.experiences });
  }
};
...
export const getEducation = async (req, res) => {
  try {
    if (mongoose.connection.readyState === 1) {
      const dbEdu = await Education.find();
      if (dbEdu.length > 0) return res.json({ success: true, data: dbEdu });
    }
    return res.json({ success: true, data: fallbackData.education });
  } catch (error) {
    return res.json({ success: true, data: fallbackData.education });
  }
};
...
export const getSkills = async (req, res) => {
  try {
    if (mongoose.connection.readyState === 1) {
      ...
      if (dbSkills.length > 0) return res.json({ success: true, data: dbSkills });
    }
    return res.json({ success: true, data: fallbackData.skills });
  } catch (error) {
    return res.json({ success: true, data: fallbackData.skills });
  }
};
